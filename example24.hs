...
data Predicate = Is    Var Value             -- var has specific value
               ...
               | Or    Predicate Predicate   -- at lease one is true
...
-- allows use to accept partial solutions, then we can use a value of
...
-- Get the first solutions to the problem, by evaluating the solver computation with
-- an inital problem state and then returning the first solution in the result list,
...
-- computation with an initail problem state.
...
-- Test consistency over all allowed settings of variable.
...
main = do let variables = []
              constraints = [ Not (Equal "parent1" "parent2"),"parent1" `said` ("child" `said` ("child" `Is` "male")),saidBoth "parent2" ("child" `Is` "female") ("child" `lied` ("child" `Is` "male"))]
