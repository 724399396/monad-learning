...
-- resolve a Definiton and produce a (name, value) pair
resolveDef :: Definition -> ReaderT Environment IO (String, String)
resolveDef (D t d) = do name <- resolve t
                        value <- resolve d
                        return (name, value)

-- resolve a template into a string
resolve :: Template -> ReaderT Environment IO (String)
resolve (T s)    = return s
resolve (V t)    = do varName  <- resolve t
                      varValue <- asks (lookupVar varName)
                      return $ maybe "" id varValue
resolve (Q t)    = do teplName <- resolve t
                      body      <- asks (lookupTemplate teplName)
