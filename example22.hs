...
filterOne rules packet = do rule <- return (match rules packet)
                            case rule of
                              Noting -> do logMsg ("DROPPING UNMATCHED PACKET: " ++ (show packet)
...
-- the file named in the second argument, and the print the accepted packets followed by
...
main = do args       <- getArgs
          ...
          (out,log) <- runWriteT (filterAll rules packets)
